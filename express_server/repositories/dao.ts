...
export default class {
  static setupDbForDev() {
    db.serialize(function () {
      ...
      const dropUsagesTable = "DROP TABLE IF EXISTS usages";
      db.run(dropUsagesTable);
      ...
      const createUsagesTable =
        "CREATE TABLE IF NOT EXISTS usages (id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, cpu_hours NUMERIC, userID NUMERIC)";
      db.run(createUsagesTable);
      let password = "123";

      bcrypt.hash(password, saltRounds, function (err, hash) {
        const insertUsers = `INSERT INTO users (email, password) VALUES ('123@gmail.com', '${hash}');`;
        db.run(insertUsers);
      });
